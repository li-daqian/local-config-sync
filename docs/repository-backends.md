...
type RepositoryType = "git" | "local-folder" | "webdav" | "s3";
```
...
```ts
interface RepositoryDriver<TConfig extends RepositoryConfig> {
  readonly type: TConfig["type"];

  prepare(repository: TConfig): Promise<LocalWorkspace>;
  inspect(context: InspectContext<TConfig>): Promise<RepositoryStatus>;
  pull(context: PullContext<TConfig>): Promise<PullResult>;
  push(context: PushContext<TConfig>): Promise<PushResult>;
  doctor(repository: TConfig): Promise<DiagnosticResult>;
}
```
